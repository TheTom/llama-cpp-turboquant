        ...
        if (cparams.fused_gdn_ar) {
            ...
            for (int i = 0; i < ggml_graph_n_nodes(gf); i++) {
                ...
                if (device_gdn != device_kv) {
                    LLAMA_LOG_WARN(
                        "%s: layer %d is assigned to device %s but the fused Gated Delta Net tensor "
                        "is assigned to device %s (usually due to missing support)\n",
...
bool llama_context::ensure_sched_mtp() {
    ...
    sched_mtp.reset(ggml_backend_sched_new(backend_ptrs.data(), backend_buft.data(), backend_ptrs.size(), max_nodes,
                                           /*pipeline_parallel*/ false, cparams.op_offload));
    if (!sched_mtp) {
...
llm_graph_result * llama_context::process_ubatch_mtp(const llama_ubatch &     ubatch,
...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch &     ubatch,
    ...
    const llm_graph_params gparams = gtype == LLM_GRAPH_TYPE_DECODER_MTP ? graph_params_mtp(res, ubatch, mctx) :
...
int llama_context::encode(const llama_batch & batch_inp) {
    ...
    if (embd_pre_norm.data && t_h_pre_norm && cparams.pooling_type == LLAMA_POOLING_TYPE_NONE) {
        ...
        ggml_backend_tensor_get_async(backend_h, t_h_pre_norm, embd_pre_norm.data, 0,
    ...
    do {
        ...
        // only meaningful in LLAMA_POOLING_TYPE_NONE (per-token); other pooling modes are ignored.
        {
            ...
            if (embd_pre_norm.data && t_h_pre_norm && n_rows > 0 && cparams.pooling_type == LLAMA_POOLING_TYPE_NONE) {
                ...
                ggml_backend_tensor_get_async(backend_h, t_h_pre_norm, embd_pre_norm_out, 0,
...
uint32_t llama_context::output_reserve(int32_t n_outputs) {
    ...
    const size_t new_size  = (logits.size + embd.size + embd_pre_norm.size + backend_float_count) * sizeof(float) +
    ...
    embd_pre_norm = has_embd_pre_norm ? buffer_view<float>{ (float *) (base + offset), embd_pre_norm.size } :
...
llm_graph_params llama_context::graph_params_mtp(llm_graph_result *             res,
                                                 const llama_ubatch &           ubatch,
...
int32_t llama_context::decode_mtp(llama_seq_id  seq_id,
                                  llama_pos     attn_pos,
    ...
    if (out_logits) {
        ...
        return decode_mtp_sync(seq_id, attn_pos, last_token, h_prev, n_steps, out_drafts, out_logits, out_h_prev_last);
    }
...
int32_t llama_context::decode_mtp_sync(llama_seq_id  seq_id,
                                       llama_pos     attn_pos,
                                       llama_token   last_token,
                                       float *       h_prev,
                                       int32_t       n_steps,
    ...
    for (int32_t k = 0; k < n_steps; ++k) {
        ...
        if (out_logits) {
            ...
            ggml_backend_tensor_get(t_logits, out_logits + (int64_t) k * n_vocab, 0, (size_t) n_vocab * sizeof(float));
        }
...
int32_t llama_context::decode_mtp_async(llama_seq_id  seq_id,
                                        llama_pos     attn_pos,
                                        llama_token   last_token,
                                        const float * h_prev,
                                        int32_t       n_steps) {
...
int32_t llama_context::decode_mtp_wait(llama_token * out_drafts, float * out_h_prev_last) {
    std::unique_lock<std::mutex> lk(mtp_mu);
    mtp_cv_response.wait(lk,
                         [this] { return mtp_completed.has_value() || (!mtp_in_flight && !mtp_pending.has_value()); });
...
void llama_context::mtp_worker_loop() {
    for (;;) {
        mtp_request req;
        {
            std::unique_lock<std::mutex> lk(mtp_mu);
            mtp_cv_request.wait(
                lk, [this] { return mtp_worker_stop.load(std::memory_order_acquire) || mtp_pending.has_value(); });
...
llama_context * llama_init_from_model(llama_model * model, llama_context_params params) {
    ...
    if (params.ctx_type == LLAMA_CONTEXT_TYPE_MTP && model->hparams.nextn_predict_layers == 0) {
...
int32_t llama_decode_mtp(llama_context * ctx,
                         llama_seq_id    seq_id,
                         llama_pos       attn_pos,
                         llama_token     last_token,
                         float *         h_prev,
...
int32_t llama_decode_mtp_async(llama_context * ctx,
                               llama_seq_id    seq_id,
                               llama_pos       attn_pos,
                               llama_token     last_token,
                               const float *   h_prev,
...
int32_t llama_decode_mtp_wait(llama_context * ctx, llama_token * out_drafts, float * out_h_prev_last) {
    if (!ctx) {
        LLAMA_LOG_ERROR("%s: ctx is NULL\n", __func__);
        return -1;
    }
