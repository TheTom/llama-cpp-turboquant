...
int32_t llama_triattention_init(
    ...
    // Qwen3.8 hybrid: recurrent memory owns a separate attention KV cache.
    if (!kv) {
        if (auto * hybrid = dynamic_cast<llama_memory_hybrid *>(mem)) {
            kv = hybrid->get_mem_attn();
            if (kv) {
                LLAMA_LOG_INFO("%s: hybrid memory detected; using attention KV cache for TriAttention\n", __func__);
            }
        }
    }

    if (!kv) {
        LLAMA_LOG_ERROR("%s: no attention KV cache found\n", __func__);
        return -1;
    }
