...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ggml_init_params params = {
                // 3 tensors per layer per stream (K, V, and optional K_idx), plus one view per tensor per stream,
                // plus +3 for turbo rotation matrices (turbo_rotation + turbo_rotation_inv + turbo_innerq_scale_inv).
    ...
    if (other) {
        ...
        // always create Hadamard rotation tensors for DeepSeek-family DSA lightning
