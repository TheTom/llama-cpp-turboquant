    ...
    if (triattention_st != nullptr) {
        ...
        if (triattention_st->prefix_length == 0 && ubatch.n_tokens > 1) {
            ...
            for (uint32_t i = 0; i < ubatch.n_tokens; ++i) {
                ...
                if (ubatch.pos[i] > max_batch_pos) {
                    max_batch_pos = ubatch.pos[i];
                }
            }
            if (has_pos_zero) {
        ...
        // Count occupied cells in the primary attention stream.
        uint32_t n_used = 0;
...
void llama_kv_cache::init_triattention(const char * stats_path, const triattention_config * cfg) {
    ...
    triattention_st = triattention_init(
        stats_path, cfg, kv_size, rope_theta, head_dim, n_kv_heads);
