...
void llama_model_deepseek4::load_arch_hparams(llama_model_loader & ml) {
    ...
    if (!ml.get_key_or_arr(LLM_KV_SWIGLU_CLAMP_EXP,   hparams.swiglu_clamp_exp,   hparams.n_layer(), false)) {
        std::fill(hparams.swiglu_clamp_exp.begin(), hparams.swiglu_clamp_exp.begin() + hparams.n_layer(), 0.0f);
    }
...
ggml_tensor * llama_model_deepseek4::graph::build_attention(
    ...
    if (ratio == DSV4_HCA_RATIO && inp_dsv4->get_hca().state_pos) {
        ...
        if (layer.attn_comp_ape) {
            ggml_tensor * ape_rows = ggml_get_rows(ctx0, layer.attn_comp_ape, inp_dsv4->get_hca().state_pos);
    ...
    if (ratio == DSV4_CSA_RATIO && inp_dsv4->get_csa().state_pos && inp_dsv4->get_csa().state_write_idxs) {
        ...
        if (layer.attn_comp_ape) {
            ggml_tensor * csa_ape_rows = ggml_get_rows(ctx0, layer.attn_comp_ape, inp_dsv4->get_csa().state_pos);
        ...
        if (inp_dsv4->get_csa().state_persist_src_idxs) {
        ...
        if (layer.indexer_comp_ape) {
            ggml_tensor * lid_ape_rows = ggml_get_rows(ctx0, layer.indexer_comp_ape, inp_dsv4->get_lid().state_pos);
        ...
        if (inp_dsv4->get_lid().state_persist_src_idxs) {
    ...
    if (ratio == DSV4_HCA_RATIO && inp_dsv4->get_hca().state_pos) {
        ...
        if (inp_dsv4->get_hca().state_persist_src_idxs) {
        ...
        hca_state_kv = nullptr;
        hca_state_score = nullptr;
    }
