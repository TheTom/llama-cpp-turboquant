...
ggml_tensor * llm_graph_context::build_attn_mha_chrono(
         ggml_tensor * q,
    ...
    GGML_UNUSED(il);
    const auto * mctx_cur = static_cast<const llama_kv_cache_context *>(mctx);
    const auto n_stream = k_anchor->ne[3];

    q = ggml_view_4d(ctx0, q, q->ne[0], q->ne[1], q->ne[2]/n_stream, n_stream, q->nb[1], q->nb[2], q->nb[3]/n_stream, 0);
    q = ggml_permute(ctx0, q, 0, 2, 1, 3);

    ggml_tensor * cur = ggml_flash_attn_ext_chrono(
            ctx0,
            q,
            k_anchor,
            k_delta,
            ...
            kq_mask,
            kq_scale,
            (int32_t) mctx_cur->chrono_stride_k(),
            (int32_t) mctx_cur->chrono_stride_v());
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    ggml_tensor * q = q_cur;
    const bool use_chrono_fused =
        mctx_cur->chrono_fused_enabled(il) &&
        q_cur->ne[2] == 1 &&
        kq_b == nullptr &&
        sinks == nullptr &&
        v_mla == nullptr &&
    ...
    // store to KV cache
    {
        ...
        ggml_build_forward_expand(gf, mctx_cur->cpy_k(ctx0, k_cur, k_idxs, il, !use_chrono_fused));
        ggml_build_forward_expand(gf, mctx_cur->cpy_v(ctx0, v_cur, v_idxs, il, !use_chrono_fused));
    }

    ggml_tensor * cur;
    if (use_chrono_fused) {
        cur = build_attn_mha_chrono(
                q,
                mctx_cur->get_chrono_k_anchor(ctx0, il),
                mctx_cur->get_chrono_k_delta(ctx0, il),
                mctx_cur->get_chrono_k_scale(ctx0, il),
                mctx_cur->get_chrono_v_anchor(ctx0, il),
                mctx_cur->get_chrono_v_delta(ctx0, il),
                mctx_cur->get_chrono_v_scale(ctx0, il),
                kq_mask,
                kq_scale,
                il);
    } else {
