...
//
// edit_file_lines: line-range based file edit (replace/delete/append), one or
// more changes per call. Complements edit_file's exact-text-match approach
// for callers that prefer to address content by line number.
//

struct server_tool_edit_file_lines : server_tool {
    server_tool_edit_file_lines() {
        name = "edit_file_lines";
        display_name = "Edit file (line ranges)";
        permission_write = true;
    }
    ...
    json invoke(json params, server_tool::stream *) const override {
        ...
        // split into lines (line ending stripped; reconstructed with '\n' on write)
        std::vector<std::string> lines;
        {
            std::istringstream fin(original_content);
            std::string line;
        ...
        // reassemble and write file back
        ...
        return {{"result", "file edited successfully"}, {"path", path}, {"changes_applied", (int) entries.size()}};
    }
};
...
static std::vector<std::unique_ptr<server_tool>> build_tools() {
    ...
    tools.push_back(std::make_unique<server_tool_edit_file_lines>());
