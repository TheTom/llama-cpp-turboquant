...
| GGML_CUDA_FA_ALL_QUANTS       | Boolean                | false   | Compile support for all KV cache quantization type (combinations) for the FlashAttention CUDA kernels. More fine-grained control over KV cache size but compilation takes much longer. Also required for block KV streaming (`--kv-stream-arena-mib`) to use each type's native attention kernel instead of always dequantizing streamed pages to F16 first; without it, streaming still works for every supported type, just via the F16 fallback path.                                                                                                                                                                                           |
