...
#undef SET_ROWS
    ...
    // TQ: turbo KV-cache set_rows pipelines are F32-src only (the KV cache is always
    // written from F32 activations), so these live outside the generic src-parameterized
    // SET_ROWS macro above rather than being instantiated (and needing SPIR-V) for f16-src too.
...
static ggml_status ggml_backend_vk_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    // Also submit at least every ctx->device->max_nodes_per_submit nodes, in case there
    // are workloads without heavy compute. This flops-based heuristic (vs. the fork's
    // former matmul-bytes-only one) also accounts for flash-attention-heavy graphs (e.g.
    // a dense MTP head doing full attention over deep KV), which previously could
    // accumulate past the GPU job watchdog timeout (issue #185; cf. upstream
    // ggml-org/llama.cpp#21724) since byte-counting only tracked matmul work.
