...
void process_shaders() {
    ...
    // TQ: turbo KV-cache set_rows shaders are F32-src only (see the SET_ROWS_TURBO
    // pipeline registration in ggml-vulkan.cpp for why these are generated separately
    // from the generic src x dst loop above).
    for (std::string t : {"turbo2_0", "turbo3_0", "turbo4_0"}) {
        string_to_spv("set_rows_f32_" + t + "_i32", "copy_to_quant.comp", {{"SET_ROWS", "1"}, {"DATA_A_" + to_uppercase(t), "1"}, {"B_TYPE", "uint"}, {"B_SIZE", "32"}, {"S_TYPE", "float"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}});
        string_to_spv("set_rows_f32_" + t + "_i64", "copy_to_quant.comp", {{"SET_ROWS", "1"}, {"DATA_A_" + to_uppercase(t), "1"}, {"B_TYPE", "uvec2"}, {"B_SIZE", "64"}, {"S_TYPE", "float"}, {"D_TYPE", "float"}, {"FLOAT_TYPE", "float"}});
    }
