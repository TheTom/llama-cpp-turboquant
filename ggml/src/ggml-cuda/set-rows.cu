...
// ---- TURBO6 set_rows: 128-element groups with WHT rotation + 6-bit quantization ----
//
// Same shape as k_set_rows_turbo4: block size IS the WHT group size (128), so 1 CUDA
// block = 1 turbo6 block and thread j handles element j. Only the codebook (64 centroids)
...
#pragma unroll
    ...
    const float rv = x[j];
    const uint8_t idx = turbo6_nearest_centroid(rv);
    ...
    // ---- Step 7: Reconstruction norm (parallel) ----
    const float c = TURBO6_CENTROIDS[idx];
    float rc = c * c;
    for (int offset = WARP_SIZE / 2; offset > 0; offset >>= 1)
        rc += __shfl_xor_sync(0xffffffff, rc, offset, WARP_SIZE);
    if (j % WARP_SIZE == 0)
...
// ---- TURBO5 set_rows: 128-element groups with WHT rotation + 5-bit quantization ----
//
// Same shape as k_set_rows_turbo4: block size IS the WHT group size (128), so 1 CUDA
// block = 1 turbo5 block and thread j handles element j. Only the codebook (32 centroids)
// and the packing differ: the low nibble goes to qs[] exactly like turbo4, the high
...
#pragma unroll
    ...
    const float rv = x[j];
    const uint8_t idx = turbo5_nearest_centroid(rv);
    ...
    // Each 8-lane group gathers its sign bits via a width-32 shuffle, so the group is
    // self-contained on wave64 (__ballot_sync truncates the 64-bit mask to uint32_t there).
    const uint8_t my_nibble = turbo5_code_to_mag(idx);
    const uint8_t my_high   = turbo5_code_to_neg(idx);
    const uint8_t partner_nibble = __shfl_sync(0xffffffff, my_nibble, lane ^ 1, WARP_SIZE);
    if (j % 2 == 0) {
        blk->qs[j / 2] = my_nibble | (partner_nibble << 4);
    }
...
#pragma unroll
    ...
    // ---- Step 7: Reconstruction norm (parallel) ----
    const float c = TURBO5_CENTROIDS[idx];
    float rc = c * c;
    for (int offset = WARP_SIZE / 2; offset > 0; offset >>= 1)
        rc += __shfl_xor_sync(0xffffffff, rc, offset, WARP_SIZE);
    if (j % WARP_SIZE == 0)
...
static void set_rows_cuda_turbo6(
    ...
    GGML_TENSOR_BINARY_OP_LOCALS
    GGML_ASSERT(ne00 % QK_TURBO6 == 0);  // must be block-aligned (128)

    cudaStream_t stream = ctx.stream();

    // turbo6 block size = WHT group size = 128, always
    ...
    // InnerQ: check/finalize calibration before kernel launch
    turbo_innerq_check_finalize(QK_TURBO6, ne00);
...
static void set_rows_cuda_turbo5(
    ...
    GGML_TENSOR_BINARY_OP_LOCALS
    GGML_ASSERT(ne00 % QK_TURBO5 == 0);  // must be block-aligned (128)

    cudaStream_t stream = ctx.stream();

    // turbo5 block size = WHT group size = 128, always
    ...
    // InnerQ: check/finalize calibration before kernel launch
    turbo_innerq_check_finalize(QK_TURBO5, ne00);
