...
std::vector<common_trie::prefix_and_next> common_trie::collect_prefix_and_next() const {
    std::vector<uint32_t> prefix;
    std::vector<prefix_and_next> result;
    std::function<void(size_t)> collect = [&](size_t index) {
        if (nodes[index].pattern < 0) {
            if (!nodes[index].children.empty()) {
                ...
                result.emplace_back(prefix_and_next{prefix, chars});
            }
        }
